...
Note: modified version of scroll-converter.js file to use
on an element (#horizontal-scroll) instead of whole page
*/
...
window.scrollConverter = (function (window, document, undefined) {
	...
	var docElem = document.documentElement,
		scrollElem = document.getElementById('horizontal-scroll'),
		active = false,
	...
	var scrollCallback = function (offset, event, callback) {
			...
			scrollWidth = scrollElem.scrollWidth || 0;
			...
			if (newOffset >= 0 && newOffset <= maxOffset) {
				...
				scrollElem.scrollTo(offset.x, offset.y);
			}
		...
		bindEvents = function (offset, cb) {
			...
			if (scrollElem.addEventListener) {
				...
				if ("onwheel" in window) {
					scrollElem.addEventListener("wheel", mouseWheelHandler, useCaptureOrOptions);
				}
				// Old browsers
				else if ("onmousewheel" in window) {
					scrollElem.addEventListener("mousewheel", mouseWheelHandler, useCaptureOrOptions);
				}
				// Old Firefox
				else {
					scrollElem.addEventListener("DOMMouseScroll", mouseWheelHandler, useCaptureOrOptions);
				}

				scrollElem.addEventListener("scroll", scrollHandler, useCaptureOrOptions);
			}
			// IE8 and below
			else {
				scrollElem.attachEvent("onmousewheel", mouseWheelHandler);
				scrollElem.attachEvent("onscroll", scrollHandler);
			}
		},
